<?php	
	...
	if($campeonato['jogador_por_time'] == 1){ // INSCRIÇÕES SOLO
		...
		mysqli_query($conexao, "INSERT INTO notificacao VALUES (NULL, '$notificacao', $jogador, 0)");
	}else{ // INSCRIÇÕES DE TIME
		...
		switch($_GET['funcao']){ 
			case 1: // APROVAR INSCRIÇÃO
				mysqli_query($conexao, "UPDATE campeonato_inscricao SET status = 1 WHERE cod_campeonato = ".$campeonato['codigo']." AND cod_equipe = ".$inscricao['cod_equipe']."");
				$msg = "<a href='campeonato/".$campeonato['codigo']."/inscricoes/'><li>A inscrição de sua equipe no torneio <strong>".$campeonato['nome']."</strong> foi aprovada. Nos vemos dia ".date("d/m/Y", strtotime($campeonato['inicio']))." às ".date("H:i", strtotime($campeonato['inicio']))."</li></a>";
				break;
			case 2: // RECUSAR INSCRIÇÃO
				mysqli_query($conexao, "UPDATE campeonato_inscricao SET status = 2 WHERE cod_campeonato = ".$campeonato['codigo']." AND cod_equipe = ".$inscricao['cod_equipe']."");
				$notificacao = "<a href='campeonato/".$campeonato['codigo']."/inscricoes/'><li>A inscrição de sua equipe no torneio <strong>".$campeonato['nome']."</strong> foi recusada. Se discorda dessa decisão, entre em contato com a organização através do e-mail: <strong>".$organizacao['email']."</strong></li></a>";
				break;
		}
		$destinos = mysqli_query($conexao, "SELECT * FROM campeonato_lineup WHERE cod_campeonato = ".$campeonato['codigo']." AND cod_equipe = ".$inscricao['cod_equipe']."");
		while($destino = mysqli_fetch_array($destinos)){
			mysqli_query($conexao, "INSERT INTO notificacao VALUES (NULL, '$notificacao', ".$destino['cod_jogador'].", 0)");
		}
	}
